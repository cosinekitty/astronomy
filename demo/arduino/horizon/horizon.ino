...
void loop() {
    ...
    if (argcs != 4)
    {
        snprintf(printBuffer, sizeof printBuffer, "USAGE: horizon latitude longitude yyyy-mm-ddThh:mm:ssZ\n");
    ...
    astro_rotation_t rot_ecl_hor)
{
    ...
    {
        snprintf(printBuffer, sizeof printBuffer, "Altitude: error %d converting ecliptic angles to vector.\n", ecl_vec.status);
        Serial.println(printBuffer);
    }
    ...
    {
        snprintf(printBuffer, sizeof printBuffer, "Altitude: error %d rotating ecliptic vector to horizontal vector.\n", hor_vec.status);
        Serial.println(printBuffer);
    }
    ...
    *hor = Astronomy_HorizonFromVector(hor_vec, REFRACTION_NORMAL);
    ...
    {
        snprintf(printBuffer, sizeof printBuffer, "Altitude: error %d converting horizontal vector to angles.\n", hor->status);
        Serial.println(printBuffer);
    }
    return 0;   /* success */
}
...
static int FindEclipticCrossings(astro_observer_t observer, astro_time_t time)
{
    ...
    */
    ...
    {
        snprintf(printBuffer, sizeof printBuffer, "FindEclipticCrossings: error %d trying to find rotation matrix.\n", rot.status);
        Serial.println(printBuffer);
    }
    ...
    */
    ...
    for (i=0; i < NUM_SAMPLES; ++i)
    {
        ...
        if (a1*a2 <= 0.0)
        {
            ...
            snprintf(printBuffer, sizeof printBuffer,"Ecliptic longitude %9.4lf %s through horizon az %9.4lf, alt %12lg\n", ex, direction, hx.lon, hx.lat);
            Serial.println(printBuffer);
        }
    }
    return 0;
