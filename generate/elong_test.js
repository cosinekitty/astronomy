...
function TestFile(filename, startSearchYear, targetRelLon) {
    ...
    for (let item of data) {
        ...
        if (DebugMode) console.log(`JS ${item.body}: error = ${diff_minutes.toFixed(3)} minutes`);
...
function TestPlanet(outFileName, body, startYear, stopYear, zeroLonEventName) {
    ...
    if (DebugMode) console.log(`JS TestPlanet(${body}): ${count} events, interval min=${min_diff.toFixed(1)}, max=${max_diff.toFixed(1)}, avg=${(sum_diff/count).toFixed(1)}, ratio=${ratio.toFixed(3)}`);
...
function TestMaxElong(body, startText, verifyText, verifyAngle, verifyVisibility) {
    ...
    if (DebugMode) console.log(`JS TestMaxElong: ${body.padStart(8)} ${evt.visibility.padStart(8)} elong=${evt.elongation.toFixed(2).padStart(5)} (${arcmin_diff.toFixed(2).padStart(4)} arcmin)  ${evt.time.toString()} (err ${hour_diff.toFixed(2).padStart(5)} hours)`);
